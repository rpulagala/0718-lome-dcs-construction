...
import { listRequests, assignableUsers, type ListParams } from "@/lib/services/requestQueries";
...
export default async function RequestsPage({
  ...
  const [list, categories, users] = await Promise.all([
    listRequests(params),
  ...
  return (
    <>
      ...
      <main className="max-w-7xl px-6 py-8">
        ...
        {/* Filters */}
        <form method="get" className="mt-5 grid gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-3 lg:grid-cols-4">
          <input
            type="search"
            name="q"
          ...
          <div className="flex gap-2">
            ...
            <Link href="/requests" className="flex h-9 items-center rounded-md border border-slate-300 px-4 text-sm text-slate-600 hover:bg-slate-50">
              Reset
            </Link>
          </div>
        </form>
