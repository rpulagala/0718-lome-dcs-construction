  ...
  return (
    <div className="pb-10">
      ...
      {threads.length === 0 ? (
        ...
        <div className="mx-5 divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          {threads.map((t) => (
            <Link
              key={t.requestId}
            ...
            >
              <div className="flex h-11 w-11 flex-none items-center justify-center rounded-full bg-brand-navy/10 text-brand-navy">
                <MessagesSquare className="h-5 w-5" aria-hidden />
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <p className="truncate text-sm font-semibold text-brand-ink">{t.title}</p>
                  {t.lastAt && <span className="flex-none text-[11px] text-slate-400">{timeLabel(t.lastAt)}</span>}
                </div>
                <p
                  className={`truncate text-xs ${t.unread > 0 ? "font-semibold text-brand-ink" : "text-slate-400"}`}
                >
