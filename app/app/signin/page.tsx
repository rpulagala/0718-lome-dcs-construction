...
export default function PortalSignIn() {
  ...
  const input =
    "mt-1 h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-base outline-none focus:border-brand-navy dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-blue-400";
  const button =
    "h-12 w-full rounded-xl bg-brand-navy text-base font-semibold text-white transition-transform hover:bg-brand-navy-dark active:scale-[0.99] disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-navy focus-visible:ring-offset-2";
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col justify-center px-6 pb-16">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-brand-ink dark:text-slate-100">DCS Construction</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Track your projects, estimates, and messages.</p>
      </div>
      ...
      {step === "email" ? (
        ...
        >
          <p className="text-sm text-slate-600 dark:text-slate-300">
            Enter the 6-digit code we sent to <span className="font-medium text-slate-900 dark:text-slate-100">{email}</span>.
          </p>
          {devCode && (
            <p
              className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:bg-amber-950/40 dark:text-amber-300"
          ...
          <input
            ...
            className="h-14 w-full rounded-xl border border-slate-300 bg-white text-center text-2xl tracking-[0.5em] outline-none focus:border-brand-navy dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-blue-400"
          />
          {error && <p className="text-sm text-red-600" role="alert">{error}</p>}
          ...
          <button
            ...
            className="h-10 w-full text-sm font-medium text-slate-500 dark:text-slate-400"
          >
