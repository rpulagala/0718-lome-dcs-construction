...
> **📱 Client app (2026-07-15, through C7):** the customer portal under `/app` uses a **separate passwordless auth** (email one-time code) with its own HMAC-signed httpOnly session cookie, distinct from the staff Auth.js session. Codes are 6-digit, hashed at rest (HMAC), single-use, 10-minute expiry, attempt-capped (5), and rate-limited per IP + per email; the request-code endpoint never reveals whether an account exists. **Data isolation is enforced and tested:** every portal query is scoped to the signed-in `customerAccountId` server-side (detail reads use `findFirst({ where: { id, customerAccountId } })` → 404 for anything unowned), and each customer-facing surface has dedicated **IDOR negative tests** (requests/detail/photos C2, messaging send/read/mark C4, **estimate accept/decline C5**), consolidated in **C7** into a single attacker-vs-victim isolation suite ([tests/integration/portalIsolation.test.ts](../tests/integration/portalIsolation.test.ts)) plus a mobile **E2E** that exercises an unowned id → 404. **C7 security review:** re-reviewed portal auth, per-account scoping, message/estimate authz, and the customer-actor audit trail (`actorId: null` + account id in metadata) — no new findings. **Known demo-only exception:** on-screen login codes (`devCode`) are surfaced while no mail provider is wired — this weakens the passwordless factor and MUST be removed when real email lands in **C8**. **C5 estimate actions:** `respondToPortalEstimate` re-checks that the estimate's request belongs to the account before any write, guards that the estimate is `SENT` and not past `expiresAt`, runs accept (→ `ACCEPTED` + auto-convert to a project) or decline (→ `DECLINED`) as a single atomic transaction, and audits with `actorId: null` (a customer isn't a staff `User`; the nullable `AuditLog.actorId`/`WorkRequestStatusHistory.changedById` record `null` with the account id in `metadata`). The portal is **still local-only** (not deployed to production).
>
