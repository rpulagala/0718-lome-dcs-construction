...
> **Status — 2026-07-15: 159 unit/integration tests + 14 Playwright E2E tests passing**, typecheck + lint clean, `next build` green. (Now includes the full client-portal suites: `portalSession` unit + `portalAuth`, `portalData` (list/detail isolation + IDOR negatives + least-data shaping), `portalRequests` (account-linked creation), `messaging` (two-way send/read + IDOR negatives), **`portalEstimates`** (accept→project + advance, decline, IDOR negative, and `SENT`/expired/already-has-a-project guards), and a **consolidated `portalIsolation`** suite (attacker-vs-victim IDOR across every portal service). Playwright adds a **mobile portal journey** (`portal.spec.ts`: intake → passwordless sign-in via the demo code → linked request → IDOR→404).)
