...
> **Build status — 2026-07-14:** Phases **0–7 complete and verified**, then **deployed to production** — **🟢 live at https://0718-lome-dcs-construction.vercel.app** (Vercel + Neon Postgres + Vercel Blob). **159 unit/integration tests + 14 Playwright E2E tests passing** (incl. the client-portal suites through C7), typecheck + lint clean, `next build` green. Local PostgreSQL runs via Docker on host port **5433**, migrated + seeded (8 users incl. the real admin, 14 categories, 5 settings, 29 work requests incl. 2 rich showcase jobs + a second in-process portal request for the app demo, photos, site visits, estimates, 4 projects). Dev login: `admin@dcs.example` / `Password123!`. Phase-by-phase progress is tracked below and in [DECISIONS.md](DECISIONS.md); deployment details in [DEPLOYMENT.md](DEPLOYMENT.md).
>
> | Phase | State |
> |---|---|
...
>
> **Client app (customer portal)** — a separate iPhone-style PWA under `/app`, planned in [CLIENT_APP_PLAN.md](CLIENT_APP_PLAN.md). Phases **C0 (design) + C1 (foundation: CustomerAccount + passwordless auth + mobile app shell + PWA) + C2 (Home dashboard, Active/Completed Projects list, and request/project detail — account-scoped tracking with strict per-customer isolation) + C3 (in-app multi-step "New request" creation, prefilled + photo upload, linked to the account and surfaced in the staff console) + C4 (two-way `ClientMessage` messaging — per-request threads, photo attachments, read state, ~8s polling; customer app + staff console panel share one thread) + C5 (in-app estimate accept/decline — account-scoped/IDOR-proof + audited; accept auto-converts the estimate to a tracked project; reflected in the staff console)** + C6 (PWA & polish — portal-scoped dark mode, loading/error/empty states, accessibility, transitions) + C7 (testing & hardening — a consolidated data-isolation/IDOR suite + a mobile portal E2E journey + security review) are **complete and verified locally** (159 unit/integration + 14 Playwright E2E, build green); **not launched for real customers**. Only **C8 (real email + notifications)** remains — the demo runs without a mail provider (on-screen login codes).
