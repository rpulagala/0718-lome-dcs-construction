...
> **Client app (C1 foundation):** a separate customer-facing mobile PWA under the `app/app/` route group (`/app`), with its own mobile shell (`components/portal/*`: `TabBar`, `LargeTitle`, `RegisterSW`, `SignOutButton`) and a self-contained session (`lib/portal/session.ts`, HMAC cookie) independent of staff Auth.js. Passwordless email-code auth in `lib/services/portalAuth.ts` + `/api/portal/auth/*`; PWA via `app/manifest.ts` (scoped `/app`) + `public/sw.js`. Shares the DB/services; `WorkRequest.customerAccountId` links a customer's requests to their `CustomerAccount`. Runs locally only (not deployed). See [CLIENT_APP_PLAN.md](CLIENT_APP_PLAN.md).
