...
- **Estimate accept/decline auto-converts (C5).** A customer can accept or decline a **SENT** estimate from the portal (`respondToPortalEstimate`, account-scoped/IDOR-proof, guarded on `SENT`/not-expired). **Accept auto-creates the project** (chosen over "mark approved, staff converts") in one atomic transaction — default project name = the request's category, which staff can rename in the console — reusing a shared `createProjectFromEstimateTx` extracted from `projects.ts`; decline flips the estimate + request to `DECLINED`. Customer actions are audited with `actorId: null` (customers aren't staff `User`s; `AuditLog.actorId`/`changedById` are nullable) and the account id in `metadata` (new actions `estimate.customer_accept`/`_decline`). **Documents/PDF surfacing deferred** to a later polish pass; accept/decline email notifications are deferred to C8 (real email is a later phase).

## Client app (C6–C7) notes
- **Dark mode is portal-scoped, not global (C6).** The `dark` class is applied to a wrapper div under `/app` only (via `useSyncExternalStore` over localStorage, defaulting to the OS `prefers-color-scheme`), so theming never bleeds into the staff console or public marketing site (which have no dark styling). Components only *add* `dark:` variants — the light theme is unchanged. A global `prefers-reduced-motion` rule neutralizes animation/transition for accessibility.
- **IDOR coverage consolidated (C7).** Per-feature IDOR negatives (C2/C4/C5) were unified into one attacker-vs-victim suite (`portalIsolation.test.ts`) plus a mobile E2E that hits an unowned id → 404, so the "#1 priority" isolation guarantee has a single, exhaustive home.
- **Demo shortcuts are deliberate, tracked debt.** Prefilled staff/customer credentials and the on-screen passwordless code exist only so the instance is one-click to demo without a mail provider; they weaken auth and MUST be reverted (or superseded by real email) in **C8** before real customers.
