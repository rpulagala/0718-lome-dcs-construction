...
>
> **📱 Client app (customer portal) — Home/Projects tracking built, local only:** a separate mobile-first, iPhone-style PWA for customers under **`/app`**. Phases **C0 + C1 + C2 complete** — `CustomerAccount` + passwordless email-code auth, a mobile app shell (bottom tab bar, sign-in, Profile), PWA install/offline, and the **account-scoped tracking screens** (Home dashboard, Active/Completed Projects list, and a request/project detail with milestone progress, dates, team, photos, sent estimates, site visits, and updates — with strict per-customer data isolation). Not deployed yet; C3 (in-app request creation) onward pending. Plan + status: [`docs/CLIENT_APP_PLAN.md`](docs/CLIENT_APP_PLAN.md).
>
...
- [Client app plan](docs/CLIENT_APP_PLAN.md) — customer portal (iPhone-style PWA); C0/C1/C2 built
