...
## Status — 2026-07-14
**🟢 Live in production: https://0718-lome-dcs-construction.vercel.app** — Phases **0–7 complete and verified**, then **deployed** + a post-launch branding/UX pass. **129 unit/integration tests + 11 Playwright E2E tests passing**, typecheck + lint clean, `next build` green.

**Client app (customer portal) — foundation built, local only.** A separate mobile-first, iPhone-style PWA for customers lives under **`/app`** (route group `app/app/`). **Phases C0 (design) + C1 (foundation) are complete** (see [docs/CLIENT_APP_PLAN.md](docs/CLIENT_APP_PLAN.md)): new **`CustomerAccount`** + **`CustomerLoginCode`** models and a `WorkRequest.customerAccountId` link (migration `client_portal_accounts`); **passwordless email-code auth** — self-contained HMAC-signed session cookie ([lib/portal/session.ts](lib/portal/session.ts), separate from staff Auth.js), request/verify services ([lib/services/portalAuth.ts](lib/services/portalAuth.ts), rate-limited/single-use/expiring, auto-links a customer's existing requests by email on first sign-in), a 6-digit email template, and `/api/portal/auth/{request-code,verify,signout}`; a **mobile app shell** (bottom tab bar Home/Projects/Messages/Profile, iOS-style sign-in, gated Home showing linked-request counts, Projects/Messages placeholders, Profile + sign-out) in `components/portal/*`; and a **PWA** (`app/manifest.ts` scoped to `/app`, `public/sw.js`, icons). Verified in-browser on a 390×844 viewport. **Not deployed** — runs locally only; C2 (project/request tracking) onward is pending.
